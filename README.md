...
docker run -d --name molecules -p 3000:3000 --link postgres:postgres molecules
...
```
docker build -t molecules .
docker run --rm -i -t -p 3000:3000 --link postgres:postgres -v $PWD:/src molecules /bin/sh
# where the left postgres is the name of your postgres container
